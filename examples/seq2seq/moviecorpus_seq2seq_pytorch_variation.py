...
def evaluate(searcher, word2idx,idx2word, sentence, max_length):
    ...
    # words -> indexes
    indexes_batch = [indexesFromSentence(word2idx, sentence, word2idx['EOS'])]
...
word2idx = {'PAD':PAD_token,'SOS':SOS_token,'EOS':EOS_token}
idx2word = {PAD_token:'PAD',SOS_token:'SOS',EOS_token:'EOS'}
num_words = 3  # Count SOS, EOS, PAD

for word in dataset.word2count:
...
for q,a in dataset.pairs:
    new_q = torch.tensor(indexesFromSentence(word2idx,q,EOS_token))
    new_a = torch.tensor(indexesFromSentence(word2idx, a, EOS_token))
    keep_pairs.append([new_q,new_a])

dataset.pairs = keep_pairs
