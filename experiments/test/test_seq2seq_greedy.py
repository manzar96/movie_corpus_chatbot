...
def create_searcher(options, model, device):
    searcher = GreedySeq2Seq(model, options.maxseqlen, device=device)
    return searcher
...
def test(outputfolder, test_loader, model, idx2word, device):
    """
    This function creates a txt file with generated sentences (
    test_results.txt) and a pickle file containing all_generated sentences,
    all inputs and all targets in order to be given in another script for
    calculating metrics!
    ...
    out_txt = open(os.path.join(outputfolder, "test_results.txt"), "w")
    out_pickle = open(os.path.join(outputfolder, "test_results.pkl"), "wb")
    all_gen = []
    all_logits = []
    all_inputs = []
    all_targets = []
    ...
    for index, batch in enumerate(tqdm(test_loader)):
        ...
        outputs, logits = model(input1, lengths1)

        all_gen.append(outputs)
        all_logits.append(logits.squeeze(0))
        all_inputs.append(input1.squeeze(0))
        all_targets.append(input2.squeeze(0))

        gen_words = [idx2word[token.item()] for token in outputs]
        input_words = [idx2word[token.item()] for token in input1.squeeze(0)]
        target_words = [idx2word[token.item()] for token in
                            input2.squeeze(0)]
        out_txt.write("Input sentence: {}\n".format(input_words))
        out_txt.write("Target sentence: {}\n".format(target_words))
        out_txt.write("Response sentence: {}\n".format(gen_words))
        out_txt.write("=" * 10 + "\n")

    pickle.dump(obj=(all_inputs, all_logits, all_gen, all_targets),
                file=out_pickle, protocol=pickle.HIGHEST_PROTOCOL)
    out_pickle.close()
    out_txt.close()
...
if __name__ == '__main__':
    ...
    # answer ---
    searcher = create_searcher(options, model, DEVICE)
